...
SMOVE           = 1.0               # Percentage of move mixture to be sampled with stretch move
DEMOVE          = 0.0               # Percentage of move mixture to be sampled with differential evolution move
DESMOVE         = 0.0               # Percentage of move mixture to be sampled with differential evolution snooker move
NRUN            = 100               # Maximum number of runs
...
FILEROOT        = "run8"             # Root of output filenames
OLDFILEROOT     = ""                # Old fileroot which to run from
...
OLDSAMPLESFILE  = "../results/fit/"+DISCSETUP+"/run2_samples_nmc10_nwalkers28_fstd0.3_stretch0.0_de0.5_des0.5_fitgp.h5" 
SAMPLESFILE     = "../results/fit/"+DISCSETUP+"/"+OLDFILEROOT+FILEROOT+"_samples_nmc"+str(NMC)+"_nwalkers"+str(NWALKERS)+\
                  "_fstd"+str(FSTD)+"_stretch"+str(SMOVE)+"_de"+str(DEMOVE)+"_des"+str(DESMOVE)+"_fitgp_mcmillangp.h5" 
#####################################################################################
...
def lnPost(fitpars):   
        ...
        #print("     taumax_inner       = "+str(np.round(fitpars[2],3)))
        #print("     sfrdecay_inner     = "+str(np.round(fitpars[3],3)))
        #print("     sfrgrowth_inner    = "+str(np.round(fitpars[4],3)))
        #print("     fehbirth_inner     = "+str(np.round(fitpars[5],3)))
        #print("     tauenrich_inner    = "+str(np.round(fitpars[6],3)))
        #print("     fehLzsolgrad_inner = "+str(np.round(fitpars[7],3)))
        #print("     fehsig_inner       = "+str(np.round(fitpars[8],3)))
        #print("     afemu_inner        = "+str(np.round(fitpars[9],3)))
        #print("     afesig_inner       = "+str(np.round(fitpars[10],3)))
        #print("     Rpmin_inner        = "+str(np.round(fitpars[11],3)))
        #print("     Rpmax_inner        = "+str(np.round(fitpars[12],3)))
        #print("     nfallmin_inner     = "+str(np.round(fitpars[13],3)))
        #print("     nfallmax_inner     = "+str(np.round(fitpars[14],3)))
        #print("     Rsigma_inner       = "+str(np.round(fitpars[15],3)))
        #print("     sigmar0tau0_inner  = "+str(np.round(fitpars[16],3)))
        #print("     betar_inner        = "+str(np.round(fitpars[17],3)))
        #print("     sigmaz0tau0_inner  = "+str(np.round(fitpars[18],3)))
        #print("     betaz_inner        = "+str(np.round(fitpars[19],3)))
        #print("     taumax_outer       = "+str(np.round(fitpars[20],3)))
        #print("     sfrdecay_outer     = "+str(np.round(fitpars[21],3)))
        #print("     sfrgrowth_outer    = "+str(np.round(fitpars[22],3)))
        #print("     fehsmu_outer       = "+str(np.round(fitpars[23],3)))
        #print("     fehsig_outer       = "+str(np.round(fitpars[24],3)))
        #print("     afemu_outer        = "+str(np.round(fitpars[25],3)))
        #print("     afesig_outer       = "+str(np.round(fitpars[26],3)))
        #print("     Rp_outer           = "+str(np.round(fitpars[27],3)))
        #print("     nfall_outer        = "+str(np.round(fitpars[28],3)))
        #print("     sigmar0tau0_outer  = "+str(np.round(fitpars[29],3)))
        #print("     betar_outer        = "+str(np.round(fitpars[30],3)))
        #print("     sigmaz0tau0_outer  = "+str(np.round(fitpars[31],3)))
        #print("     betaz_outer        = "+str(np.round(fitpars[32],3)))
        print(" ")
                      
        # Populate potential and edf parameter dictionaries
        parsdict["densityNorm_dh"]     = fitpars[0]
        parsdict["scaleRadius_dh"]     = fitpars[1]
        #parsdict["taumax_inner"]       = fitpars[2]
        #parsdict["sfrdecay_inner"]     = fitpars[3]   
        #parsdict["fehbirth_inner"]     = fitpars[4]   
        #parsdict["tauenrich_inner"]    = fitpars[5]   
        #parsdict["fehLzsolgrad_inner"] = fitpars[6]   
        #parsdict["fehsig_inner"]       = fitpars[7]   
        #parsdict["afemu_inner"]        = fitpars[8]   
        #parsdict["afesig_inner"]       = fitpars[9]   
        #parsdict["Rpmin_inner"]        = fitpars[10]  
        #parsdict["Rpmax_inner"]        = fitpars[11]  
        #parsdict["nfallmin_inner"]     = fitpars[12]  
        #parsdict["nfallmax_inner"]     = fitpars[13]  
        #parsdict["sigmar0tau0_inner"]  = fitpars[14]  
        #parsdict["betar_inner"]        = fitpars[15]  
        #parsdict["sigmaz0tau0_inner"]  = fitpars[16]  
        #parsdict["sigmaz0tau0_inner"]  = fitpars[18]
        #parsdict["betaz_inner"]        = fitpars[19]
        #parsdict["taumax_outer"]       = fitpars[20]
        #parsdict["sfrdecay_outer"]     = fitpars[21]
        #parsdict["sfrgrowth_outer"]    = fitpars[22]
        #parsdict["fehsmu_outer"]       = fitpars[23]
        #parsdict["fehsig_outer"]       = fitpars[24]
        #parsdict["afemu_outer"]        = fitpars[25]
        #parsdict["afesig_outer"]       = fitpars[26]
        #parsdict["Rp_outer"]           = fitpars[27]
        #parsdict["nfall_outer"]        = fitpars[28]
        #parsdict["sigmar0tau0_outer"]  = fitpars[29]
        #parsdict["betar_outer"]        = fitpars[30]
        #parsdict["sigmaz0tau0_outer"]  = fitpars[31]
        #parsdict["betaz_outer"]        = fitpars[32]
...
def main():
    ...
    if (RESTART):
        backend = emcee.backends.HDFBackend(OLDSAMPLESFILE)  
        start   = None
    ...
    if (RUNMODE=="serial"):
        ...
        sampler   = emcee.EnsembleSampler(NWALKERS,        # the number of Goodman & Weare “walkers"
                                          ...
                                          moves=[(emcee.moves.StretchMove(live_dangerously=True),SMOVE), 
                                                 (emcee.moves.DEMove(),DEMOVE),
                                                 (emcee.moves.DESnookerMove(),DESMOVE),],
                                          backend=backend)
