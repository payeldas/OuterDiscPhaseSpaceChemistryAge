"""
GENERATE LAMOST DR4 RED CLUMPS SELECTION FUNCTION

This module calculates the selection function for the LAMOST red clumps given 
Ra, Dec, [Fe/H] and age.

Example:
    Initialize as lsf = GenLamostSelFunc(ramin,ramax,decmin,decmax,kmin,kmax,jminkmin,jminkmax)
...
class GenLamostSelFunc:
    ...
    def  __init__(self,ramin,ramax,decmin,decmax,kmin,kmax,jminkmin,jminkmax):
        ...
        # Artificially make array for metallicity Z because not shared with Parsec
        # isochrones
...
#%%
sf = GenLamostSelFunc(ramin,ramax,decmin,decmax,magmin,magmax,colmin,colmax)
...
with open("../results/selfunc/selfunc_with_redclumpsel_highres.dill", "wb") as output:
    dill.dump(sf.sfinterp, output, dill.HIGHEST_PROTOCOL)  
print("...done.")         
print(" ")
       
#%% UNDILL CLASS FOR TESTING
print("Undilling selection function...")
with open("../results/selfunc/selfunc_with_redclumpsel_highres.dill", "rb") as input:
     sfinterp = dill.load(input)
print("...done.")
print(" ")

#%% CREATE LAMOST SELECTION FUNCTION MAP FOR A SELECTION OF SKY POSITIONS
nmh       = 10
mhgrid    = np.linspace(mhmin,mhmax,nmh)
ns        = 20
sgrid     = np.linspace(smin,smax,ns)
nage      = 20
...
plt.rc('font',family='serif')
for jmh in range(nmh):
    ra4d    = np.zeros([nra,ndec,nage,ns])
    dec4d   = np.zeros([nra,ndec,nage,ns])
    age4d   = np.zeros([nra,ndec,nage,ns])
    s4d     = np.zeros([nra,ndec,nage,ns])
    sf4d    = np.zeros([nra,ndec,nage,ns]) 
    for jra in range(nra):
        for jdec in range(ndec):
            for jage in range(nage):
                ratmp                  = np.stack([ragrid[jra]]*ns)
                ra4d[jra,jdec,jage,:]  = ratmp
                dectmp                 = np.stack([decgrid[jdec]]*ns)
                dec4d[jra,jdec,jage,:] = dectmp
                s4d[jra,jdec,jage,:]   = sgrid
                mhtmp                  = np.stack([mhgrid[jmh]]*ns)
                agetmp                 = np.stack([agegrid[jage]]*ns)
                age4d[jra,jdec,jage,:] = agetmp
                sf4d[jra,jdec,jage,:]  = sfinterp((ratmp,dectmp,sgrid,mhtmp,agetmp))
    ...
    for jra in range(0,nra,nra/(nra_plot-1)):
        ...
        for jdec in range(0,ndec,ndec/(ndec_plot-1)):
            axarr[jra_plot,jdec_plot].set_xscale("log")
            axarr[jra_plot,jdec_plot].set_yscale("log")
            im = axarr[jra_plot,jdec_plot].contourf(age4d[jra,jdec,:,:],
                                                    s4d[jra,jdec,:,:],
                                                    sf4d[jra,jdec,:,:]/sf4d.max(),
                                                    100,colormap='YlGnBu')
            if (jra_plot==nra_plot-1):
                axarr[jra_plot,jdec_plot].set_xlabel(r"$\delta$="+str(np.round(decgrid[jdec],1))+"$^o$",fontsize=10)
            if (jdec_plot==0):
            ...
            jdec_plot+=1
        jra_plot +=1
    axarr[0,2].set_title("[M/H] = "+str(np.round(mhgrid[jmh],2)),fontsize=16)
    plt.gcf().text(0.5,0.03,r"$\tau$ (Gyr)",fontsize=14)
    plt.gcf().text(-0.02,0.5,r"$s$ (kpc)",fontsize=14,rotation=90)
    cb = fig.add_axes([0.92, 0.125, 0.02, 0.75])
    fig.colorbar(im,cax=cb)
    plotfile = "../plots/LamostSelFunc_mh"+str(np.round(mhgrid[jmh],3))+".eps"
    fig.savefig(plotfile,format='eps')